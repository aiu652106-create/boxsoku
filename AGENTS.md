...
For fighter rankings, obey the user's explicitly selected ranking source. When Boxing Mobile is specified, use the exact Boxing Mobile organization-and-division ranking page, record the displayed announcement date, and never substitute a sanctioning-body page or another ranking site. Compare every ranking in the article body and fight-card fields against that page after saving; if Boxing Mobile does not show a current rank, omit the rank or state the page date instead of inferring it.

For article prose, run a natural-language edit before publishing: remove repeated source attributions, recap-style restatements, generic transitions, and sentences that merely announce what the next section contains. Prefer concrete openings, varied sentence length, and the concise voice of a boxing news writer. Re-read the final body as continuous prose while keeping every factual claim traceable to its source.

Before changing published article prose, paste the exact proposed replacement text into the conversation and wait for the user's explicit approval. Treat that approval as applying only to the displayed wording and named section; do not rewrite adjacent copy during the same save unless it was also shown and approved.
